...
public class App {
    ...
    public static void main(String[] args) {
        ...
        check("Add existing item increases quantity", cart.getItemCount() == 1 && cart.getTotalPrice() == 30.0); // BOGO:
                                                                                                                 // pay
                                                                                                                 // for
                                                                                                                 // 3
        ...
        check("Add item to meet bulk discount threshold",
                cart.getItemCount() == 2 && cart.getTotalPrice() == 30.0 + (30.0 * 0.9)); // 30 + 27 = 57.0

        cart.addItem("P999", 1); // Non-existent product
        check("Adding non-existent product does not change cart",
                cart.getItemCount() == 2 && cart.getTotalPrice() == 57.0);

        cart.addItem("P003", 0); // Invalid quantity
        check("Adding item with zero quantity does not change cart",
                cart.getItemCount() == 2 && cart.getTotalPrice() == 57.0);

        cart.removeItem("P002"); // Remove soda
        check("Remove item correctly updates count and price",
                cart.getItemCount() == 1 && cart.getTotalPrice() == 30.0);

        cart.removeItem("P999"); // Remove non-existent item
        check("Removing non-existent item does not change cart",
                cart.getItemCount() == 1 && cart.getTotalPrice() == 30.0);
        ...
        pricingService.addStrategy("P001", new BulkDiscountStrategy(3, 0.50)); // เปลี่ยนโปรโมชัน Apple เป็นซื้อ 3 ลด
                                                                               // 50%
